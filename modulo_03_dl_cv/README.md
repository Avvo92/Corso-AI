...
| 03 | `03_loss.py` ✅ pieno | LOSS (BCE), MSE, perché BCE per classificazione binaria, clip bilaterale, soglia 0.5. Pipeline `valuta_rete_random` + mini-progetto `valuta_modello_completo`. 12 mini-inline + 17 TODO. | 6/10 | CPU locale |
| 04 | `04_derivate_gradiente.py` ✅ pieno | Derivata come pendenza, derivata sigmoid (max 0.25, vanishing teaser), derivata ReLU (step, dying ReLU), gradiente come vettore. Semplificazione miracolosa BCE+sigmoid → `dL/dz = p - y`. Pipeline `derivate_check_completo` + mini-progetto `analizza_funzione_attivazione`. 16 mini-inline + 16 TODO. | 6-7/10 | CPU locale |
| 05 | `05_chain_rule_gd.py` ✅ pieno | Chain rule a 2/3/n livelli, chain rule QUALITATIVA su rete 2-layer (le 5 derivate per dL/dW1), gradient descent 1D e nD, effetto LR, piano dei pesi. Pipeline `addestramento_via_gradiente_numerico` (mini-neurone) + mini-progetto `confronto_lr_su_addestramento`. 17 mini-inline + 16 TODO. | 7-8/10 | CPU locale |
| 06 | `06_backprop_training.py` ✅ pieno | Forward con cache, backward step-by-step (i 5 step), `sanity_check_grad` numerico vs analitico, training loop. Pipeline `train_rete_2_layer_completo` + **mini-progetto finale `train_rete_su_csv_m2`** (rete addestrata vs LogReg M2) + **CONFRONTO PRIMA/DOPO cap.01-06**. 14 mini-inline + 19 TODO. **Pezzo più importante del primo blocco**. | 8/10 ⚠️ | CPU locale |
