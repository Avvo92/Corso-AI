...
# modello.fit(X_train, y_train)
#
# Il problema di questo codice è che il modello DecisionTreeRegressor non accetta opzioni categoriche tra le feature. Prima di procedere con la fase di addestramento e predizione occorre fare one-hot encoding per la categoria citta.
#
...
print("\nEsercizio 6\n")
case['prezzo_medio_mq'] = (case['prezzo_euro'] / case['metri_quadri']).round(2)
report = case.groupby("citta").agg(
    prezzo_medio = ('prezzo_euro', lambda x: x.mean().round(2)),
    pratiche_totali = ('id', 'count'),
    prezzo_medio_mq = ('prezzo_medio_mq', lambda x: x.mean().round(2))
)

top_citta = ((report.sort_values(by='prezzo_medio_mq', ascending=False).head(1)).index)[0]
mask = case['citta'] == top_citta

filtered_case = case.loc[mask]

cols_to_drop = ['prezzo_euro', 'prezzo_medio_mq', 'citta']

X = filtered_case.drop(columns=cols_to_drop)
y= filtered_case['prezzo_euro']

X_train, X_test, y_train, y_test = train_test_split(
    X, y, test_size=.2, random_state=42
)

modello = DecisionTreeRegressor(max_depth=5, random_state=42)
modello.fit(X_train, y_train)
...
print(f"Reali Train      => {y_train.values.astype(int)}")
print(f"Previsioni Train => {y_pred_train.astype(int)}")
print(f"MAE Train        => {mae_train:.2f}")
print(f"R² Train         => {r2_train:.3f}\n")

print(f"Reali Test      => {y_test.values.astype(int)}")
print(f"Previsioni Test => {y_pred_test.astype(int)}")
print(f"MAE Test        => {mae_test:.2f}")
print(f"R² Test         => {r2_test:.3f}\n")

print(f"Reali Test     => {y_test.values.astype(int)}")
print(f"Prev. Baseline => {y_pred_baseline.astype(int)}")
print(f"MAE Baseline   => {mae_baseline:.2f}")
print(f"R² Baseline    => {r2_baseline:.3f}\n")


#in conclusione, dalle comparazioni effettuate, si evince che il modello così come configurato non possiede grande capacità previsionale. Questo non è dovuto in particolare alla scelta di focalizzarsi solamente alla citta con prezzi più alti (nel nostro caso Milano), ma al fatto che, così facendo, il dataset a disposizione per train e test si è molto ridotto. Questa mancanza di dati non ha permesso al modello di avere sufficiente casistica dalla quale poter evincere le regole generali della determinazione del prezzo. Ne consegue che se il dataset, anche se circoscritto alla sola Milano, se fosse di dimensioni adeguate potrebbe a quel punto aiutarci a allenare adeguatamente un modello previsionale efficace nella determinazione dei prezzi della città.
